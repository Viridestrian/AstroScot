...
function weatherDescription(code: number) {
  if (code === 0) return 'Clear sky'; if (code === 1) return 'Mostly clear'; if (code === 2) return 'Partly cloudy'; if (code === 3) return 'Cloudy';
  if ([45, 48].includes(code)) return 'Foggy'; if ([51, 53, 55, 56, 57].includes(code)) return 'Drizzle'; if ([61, 63, 65, 66, 67].includes(code)) return 'Rainy';
  if ([71, 73, 75, 77].includes(code)) return 'Snowy'; if ([80, 81, 82].includes(code)) return 'Rain showers'; if ([85, 86].includes(code)) return 'Snow showers';
...
function weatherIcon(code: number) {
  ...
  if ([51, 53, 55, 56, 57].includes(code)) return '🌦️'; if ([61, 63, 65, 66, 67, 80, 81, 82].includes(code)) return '🌧️';
...
function formatDate(value: string, timezone: string) {
  return new Intl.DateTimeFormat('en-US', { month: 'long', day: 'numeric', year: 'numeric', timeZone: timezone }).format(new Date(`${value}T12:00:00`));
}

function rainMessage(probability: number, hourly: WeatherResponse['hourly'], timezone: string) {
  if (probability === 0) return 'There is no chance of rain today.';
  let message = probability < 20 ? 'Rain is very unlikely today.' : probability < 40 ? 'There is a small chance of rain today.' : probability < 60 ? 'There is a good chance of rain today.' : probability < 80 ? 'It will probably rain today.' : 'Rain is very likely today.';
  const now = Date.now();
  const index = hourly.time.findIndex((time, i) => new Date(time).getTime() >= now && hourly.rain[i] > 0.05);
...
function WeatherCard({ location }: { location: AstroScotLocation }) {
  ...
  useEffect(() => {
    if (!location || (location.latitude === 0 && location.longitude === 0)) { setWeather(null); setStatus('idle'); return; }
    let cancelled = false; setStatus('loading');
    fetchWeather(location).then((data) => { if (!cancelled) { setWeather(data); setStatus('ready'); } }).catch(() => { if (!cancelled) setStatus('error'); });
...
export function App() {
  ...
  const [locationStatus, setLocationStatus] = useState<LocationStatus>('idle'); const [locationMessage, setLocationMessage] = useState('Enter your town or city. AstroScot will remember it on this device.');
  const [locationQuery, setLocationQuery] = useState(''); const [locationResults, setLocationResults] = useState<GeocodingResult[]>([]); const [showLocationSearch, setShowLocationSearch] = useState(false);
