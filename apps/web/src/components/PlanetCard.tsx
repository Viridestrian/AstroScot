...
export function PlanetCard({ location }: { location: AstroScotLocation }) {
  ...
  return <article className="info-card planet-card"><div className="card-header"><div><p className="card-eyebrow">Planets</p></div></div><div className="planet-content"><div className="planet-list">{visiblePlanets.length > 0 ? visiblePlanets.map((planet) => <div className="planet-item" key={planet.name}><div className="planet-item-title"><span aria-hidden="true">{planet.symbol}</span><strong>{planet.name}</strong></div><span>{statusLabel(planet.status)} · {planet.direction ?? 'Look up'}</span><p>{viewingWindow(planet.visibleFrom, planet.visibleUntil, conditions.sunset, location.timezone) ?? 'Look during the evening.'} {planet.bestTime ? `Best time to look will be around ${formatApproximatePlanetTime(planet.bestTime, location.timezone)}.` : ''} {planet.reason}{planet.bestAltitude !== null ? ` ${heightDescription(planet.bestAltitude)}.` : ''}</p></div>) : <div className="planet-item"><div className="planet-item-title"><span aria-hidden="true">🔭</span><strong>No easy planet targets</strong></div><span>Tonight</span><p>Nothing is high and bright enough to make an easy naked-eye evening target tonight.</p></div>}</div></div></article>;
}
